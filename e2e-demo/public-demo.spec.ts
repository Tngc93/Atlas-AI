...
async function expectEnglishDemoCopy(page: Page) {
  const visibleText = (await page.locator("body").innerText()).replace("Türkçe", "");
...
test("temporary CRUD, memory and Mock AI state reset together", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Explain my financial health." }).click();
  await expect(page.getByLabel("AI Coach response")).toBeVisible();
  ...
  await openDemoRoute(page, "AI Coach");
  await expect(page.getByLabel("AI Coach response")).toHaveCount(0);
});
...
test("forecast, simulator, reminders and Mock AI remain reversible and network-free", async ({ page }) => {
  ...
  await openDemoRoute(page, "AI Coach");
  await page.getByRole("button", { name: "Which debt should I pay first?" }).click();
  await expect(page.getByLabel("AI Coach response")).toContainText("4.25% monthly interest");
  await expect(page.getByText("Fictional data · Simulated AI response · No external AI request · No API key required")).toBeVisible();
  expect(forbiddenRequests).toEqual([]);
});

test("AI Coach supports session chat, keyboard input and language switching without API keys", async ({ page }) => {
  await page.goto("/demo/coach");
  await expect(page.getByRole("heading", { name: "Your financial snapshot is ready." })).toBeVisible();
  await expect(page.getByText("₺150,000", { exact: true })).toBeVisible();
  await expect(page.getByText("Mock AI · Demo Mode").first()).toBeVisible();
  await expect(page.getByLabel(/API key/i)).toHaveCount(0);

  const composer = page.getByLabel("Ask anything about your finances...");
  await composer.fill("Can I save ₺20,000 per month?");
  await composer.press("Enter");
  await expect(page.getByText("Can I save ₺20,000 per month?", { exact: true })).toBeVisible();
  await expect(page.getByLabel("AI Coach response")).toContainText("₺12,000");
  await expect(page.getByText("AI explanations do not change the calculations produced by the finance engine.")).toBeVisible();

  await page.getByRole("button", { name: "Clear Chat" }).click();
  await expect(page.getByLabel("AI Coach response")).toHaveCount(0);
  await expect(page.getByRole("heading", { name: "Your financial snapshot is ready." })).toBeVisible();

  await page.getByLabel("Response language").selectOption("tr");
  await expect(page.getByRole("button", { name: "Önce hangi borcu ödemeliyim?" })).toBeVisible();
  await page.getByRole("button", { name: "Önce hangi borcu ödemeliyim?" }).click();
  await expect(page.getByLabel("AI Koç yanıtı")).toContainText("Avalanche sırası");

  await page.getByRole("button", { name: "Yeni sohbet" }).click();
  await expect(page.getByLabel("Finansınız hakkında bir soru sorun...")).toBeFocused();
  await page.getByLabel("Finansınız hakkında bir soru sorun...").fill("İlk satır");
  await page.getByLabel("Finansınız hakkında bir soru sorun...").press("Shift+Enter");
  await expect(page.getByLabel("Finansınız hakkında bir soru sorun...")).toHaveValue("İlk satır\n");

  const storage = await page.evaluate(() => JSON.stringify({ local: Object.entries(localStorage), session: Object.entries(sessionStorage), url: location.href }));
  expect(storage).not.toContain("İlk satır");
  await page.reload();
  await expect(page.getByLabel("AI Koç yanıtı")).toHaveCount(0);
  await expect(page.getByLabel("Response language")).toHaveValue("en");
});
