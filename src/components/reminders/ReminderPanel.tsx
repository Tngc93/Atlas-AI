...
function ReminderActionButton({
  ...
  return (
    <form action={updateReminderStateAction}>
      ...
      <button
        type="submit"
        className="rounded-md border border-line bg-surface px-3 py-1.5 text-xs font-semibold text-steel transition hover:border-mint/35 hover:text-ink"
      >
        {children}
      </button>
    </form>
    ...
    <section className="ui-card">
      ...
      {reminders.length === 0 ? (
        <div className="mt-5 rounded-md border border-line bg-surface-muted p-4">
          <h3 className="text-sm font-semibold">Şu anda öne çıkan hatırlatma yok</h3>
          <p className="mt-2 text-sm leading-6 text-steel">
            Yaklaşan ödeme, yüksek risk veya eksik kayıt sinyali oluşursa burada görünür.
          </p>
        </div>
      ) : (
        <div className="mt-5 space-y-3">
          {reminders.map((reminder) => {
            ...
            return (
              <article key={reminder.key} className="rounded-lg border border-line bg-surface-muted p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-sm font-semibold text-ink">{reminder.title}</h3>
                      <StatusPill tone={severityTone(reminder.severity)}>{severityLabel(reminder.severity)}</StatusPill>
                    </div>
                    <p className="mt-2 text-sm leading-6 text-steel">{reminder.reason}</p>
                    <p className="mt-2 text-sm leading-6 text-steel">
                      <span className="font-semibold text-ink">Gözden geçir:</span> {reminder.suggestedReview}
                    </p>
                    {dueDate ? <p className="mt-2 text-xs font-semibold text-steel">Tarih: {dueDate}</p> : null}
                  </div>
