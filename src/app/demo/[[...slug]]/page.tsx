...
import { AlertCircle, ArrowRight, CheckCircle2, DatabaseZap } from "lucide-react";
import { useMemo, useState, type FormEvent, type ReactNode } from "react";
import { CoachChatExperience } from "@/components/coach/CoachChatExperience";
...
function CoachDemo() {
  const { planSnapshot } = useDemoFinance();
  const financialSnapshot = useMemo(() => buildCoachFinancialSnapshot(planSnapshot), [planSnapshot]);
  return <><PageHeader title="AI Coach" description="Ask questions about the fictional snapshot. Mock AI explains deterministic finance-engine outputs without any external provider request." /><CoachChatExperience mode="demo" financialSnapshot={financialSnapshot} provider={{ label: "Mock AI", available: true, isMock: true }} defaultLanguage="en" /></>;
}
